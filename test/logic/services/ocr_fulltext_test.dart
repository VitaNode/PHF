...
void main() {
  ...
  setUp(() async {
    ...
    // Initial Data
    await db.execute("INSERT INTO persons (id, nickname, created_at_ms) VALUES ('p1', 'User', 0)");
  ...
  test('Search index should aggregate text from multiple images in the same record', () async {
    // 1. Process Image 1
    when(mockSecurityHelper.decryptDataFromFile(any, any)).thenAnswer((_) async => Uint8List(0));
    when(mockOcrService.recognizeText(any, mimeType: anyNamed('mimeType'))).thenAnswer(
    ...
    expect(results.length, 1, reason: 'Should STILL find record by Image 1 text after Image 2 processed');
  });
}
