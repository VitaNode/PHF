...
void main() {
  ...
  group('OCRProcessor', () {
    ...
    test('Full success flow (High Confidence)', () async {
      ...
      when(
        mockRecordRepo.getRecordById('rec1'),
      ...
      verify(
        mockOcrService.recognizeText(decryptedBytes, mimeType: 'image/jpeg'),
      ).called(1);

      // Verify Update Image
      verify(
        mockImageRepo.updateOCRData(
          'img1',
          any,
      ...
      verify(
        mockRecordRepo.updateStatus('rec1', RecordStatus.archived),
      ).called(1);
    ...
    test('Low confidence flow (Processing -> Review)', () async {
      ...
      verify(
        mockRecordRepo.updateStatus('rec1', RecordStatus.review),
      ).called(1);
    });
