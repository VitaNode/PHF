...
- [ ] **Implement (T21.1) 自动刷新机制**:
    - 在首页与详情页实现对 OCR 队列状态的监听。当后台任务完成时，自动刷新 UI 状态（如隐藏“处理中”动画、更新“待确认”链接）。
- [x] **Implement (T21.2) 级联删除逻辑**:
    - 优化删除逻辑：当一个 Record 下的所有图片被用户手动删除后，自动删除该 Record 实体及其关联的 OCR 队列任务。
- [ ] **Implement (T21.3) OCR 质量与重新识别**:
    - 修复 OCR 结果可能出现空白的问题，增强异常捕获。
    - 在详情页增加“重新识别”功能入口，允许手动触发 OCR 流程。
- [ ] **Implement (T21.4) 详情页编辑闭环**:
    - 优化字段编辑（医院、日期）后的保存逻辑，确保 Timeline 立即同步更新。
- [ ] **Implement (T21.5) 物理擦除优化 (Technical Debt)**:
    - 针对 T10 遗留问题，优化图片临时文件的物理擦除逻辑，减少对 OS 默认删除机制的依赖，提升隐私安全性。