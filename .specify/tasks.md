...
### 5.1 模型基础设施 (Infrastructure)
- [ ] **T5.1.1**: 集成 `Mediapipe LLM Inference` (或类似绑定)。实现端侧推理引擎初始化。
- [ ] **T5.1.2**: 实现“模型按需下载”策略。用户首次启用时下载 1.5GB+ 权重，支持进度显示与断点续传。
- [ ] **T5.1.3**: 建立 **本地医学知识 SQL 库**。包含常用 200+ 检验项的标准临床意义、危急值及建议，用于 RAG 增强。

### 5.2 结构化提取与排版重构 (Logic)
- [ ] **T5.2.1**: 实现 **“结构化提取”逻辑**。不急于解读，先让 SLM 将乱序文本转换为标准 JSON (Item/Value/Unit/Status)。
- [ ] **T5.2.2**: 实现 **“历史趋势解析器”**。根据标准化项目名，检索数据库过去 3 次记录，计算趋势。
- [ ] **T5.2.3**: 开发 **“排版重构引擎”**。UI 根据 SLM 的语义块，渲染出类似于纸质资料的整齐电子报表。

### 5.3 智能解读与 UX (User Experience)
- [ ] **T5.3.1**: 实现 **Insight Card (解读卡片)**。在详情页顶部展示 AI 结论、风险提示及生活建议。
- [ ] **T5.3.2**: 开发 **对话式追问 (Medical QA)**。基于当前病历进行单/多轮提问，利用本地 RAG 确保回答稳健。
- [ ] **T5.3.3**: 设计“推理中”骨架屏。提供深度视觉反馈，缓解端侧推理（10s+）的焦虑感。

### 5.4 全球化 AI 适配
- [ ] **T5.4.1**: 适配 8 种语言的 System Prompt，确保 SLM 输出报告语言与 App 语言设置一致。
